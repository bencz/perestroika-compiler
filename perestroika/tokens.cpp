...
bool analizador_lexico(TOKEN** tokens, int max) {
	...
	for (int i = 0; i < max; i++) {
		if (tokens[i]->tipo == TOKEN_INVALIDO) {
			...
			//cout << "         na linha: " << tokens[i]->renglon << " coluna " << tokens[i]->coluna << endl;
			todoBien = false;
		}
	}
...
void tokenizar_arquivo(FILE* entrada, TOKEN** tokens, int &current) {
	char c[2];
	int leidos = 1, renglon = 1, coluna = 0;
	bool retake = false;
	bool diagonal = false;
	char anterior = 0;
	string token = "";
	ESTADO_TOKENIZER estado = TOKENIZER_ESPACO;
	c[1] = 0;
			...
			case TOKENIZER_ESPACO:
				...
				if ((c[0] >= 'a' && c[0] <= 'z') ||
					...
					estado = TOKENIZER_NUMERO;
						...
						c[0] == '%') {
					token = c;
					commit_token(token, tokens, current, renglon, coluna);
					token.clear();
				} else if (c[0] == '>' || c[0] == '=' || c[0] == '<' || c[0] == '&' ||
						c[0] == '|' || c[0] == '!') {
					token = c;
					estado = TOKENIZER_COMPARACAO;
				} else if (c[0] == '"') {
					token = c;
...
void commit_token(string token, TOKEN** tokens, int &current, int renglon, int coluna) {
	if (token.length() > 0) {
		TOKEN* obj_token = criar_token(token, renglon, coluna - token.length());
...
TIPO_TOKEN clasificar_token(string token) {
	if (token == "(") {
		...
		if (token.length() > 0 && expect_inicio_label(data[0]) == true) {
			bool label = true;
			for (unsigned int i = 1; i < token.length(); i++) {
				if (expect_caracter_label(data[i]) == false) {
					label = false;
					break;
				}
			}
...
bool expect_caracter_label(char c) {
	return expect_inicio_label(c) || (c >= '0' && c <= '9');
}
